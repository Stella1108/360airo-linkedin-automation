...
export default function AccountsContent() {
  ...
  // Fixed: Return a Promise for AccountTable
  const handleDeleteForTable = async (account: LinkedInAccount): Promise<void> => {
  ...
  // For AccountCard (non-async)
  const handleDeleteForCard = (account: LinkedInAccount) => {
    setDeleteDialog({
      open: true,
      accountId: account.id,
      accountName: account.name,
  ...
  // Add this function for automation
  const handleStartAutomation = async (account: LinkedInAccount): Promise<void> => {
    try {
      console.log('Starting automation for account:', account.name)
      // TODO: Implement automation API call
    ...
    <div className="h-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {accounts.map((account) => (
            <AccountCard
              key={account.id}
              account={account}
              onDelete={() => handleDeleteForCard(account)}
            />
          ))}
        </div>
      ) : (
      ...
      <AlertDialog open={deleteDialog.open} onOpenChange={(open) => {
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setDeleteDialog({ open: false, accountId: null, accountName: '' })}>
              Cancel
            </AlertDialogCancel>
